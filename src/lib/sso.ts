...
export function knownOrigins(): string[] {
  return [
    ...
    subOrigin("portal"),
    subOrigin("survey"),
    // The shop. It is public and signs nobody in today - but this list is the one
    // that gets forgotten, and the failure it produces is a sign-in that dead-ends
    // on an origin the front door refuses to mint for, with nothing in the UI to
    // say why. One line now, or an afternoon later.
    //
