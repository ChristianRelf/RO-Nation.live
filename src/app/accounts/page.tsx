...
export default async function CompanyAccountingPage({
  ...
  const [summary, documents, openRequests] = await Promise.all([
    accountingSummary(),
  ...
  return (
    <AccountingShell user={user} openRequests={openRequests}>
      ...
      {/* Requests come off pay.ronation.live, where a client asked for something. Called
          out here rather than left to the nav badge alone: the desk is the page staff
          open, and a queue that only announces itself in a sidebar is a queue that waits.
          Rendered only when there is something in it - an empty callout saying "nothing
          to do" is noise on every other visit. */}
      {openRequests ? (
        <Link
          href="/requests"
          className="mb-8 flex items-center justify-between gap-4 border border-accent/30 bg-accent-soft px-4 py-3 text-sm transition-colors hover:border-accent/60"
        >
          <span className="text-fg">
            <span className="font-semibold">
              {openRequests} payment request{openRequests === 1 ? "" : "s"}
            </span>{" "}
            <span className="text-muted">
              waiting on an answer, from clients on pay.ronation.live.
            </span>
          </span>
          <span className="shrink-0 text-accent">Review →</span>
        </Link>
      ) : null}
      ...
      <section className="mb-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          ...
          hint="Payroll slips and refunds, unsettled"
        />
        <StatCard
      ...
      <section className="mb-10">
        ...
        {/* A plain <a>: this one leaves the host. See the note on OutLink in
            components/accounting/accounting-shell.tsx. */}
        <a
          href={outboundUrls.payoutInvoices()}
        ...
        >
          <div className="min-w-0">
            ...
            <p className="mt-1 text-xs text-muted">
              Generated from the ticket ledger, not written by hand. Separate desk, on
              the company site.
            </p>
          </div>
